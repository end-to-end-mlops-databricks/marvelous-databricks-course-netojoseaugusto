...
with mlflow.start_run(tags={"branch": "week2", "git_sha": f"{git_sha}"}) as run:
    ...
    dataset = mlflow.data.from_spark(spark_train_set, table_name=f"{configs.get('train_uc_location')}", version="0")
    mlflow.log_input(dataset, context="training")
    conda_env = _mlflow_conda_env(
        additional_conda_deps=None,
        additional_pip_deps=[
            "package/loans-0.0.1-py3-none-any.whl",
        ],
...
model_version = mlflow.register_model(
    model_uri=f"runs:/{run_id}/pyfunc-catboost-model", name=model_name, tags={"git_sha": f"{git_sha}"}
)
